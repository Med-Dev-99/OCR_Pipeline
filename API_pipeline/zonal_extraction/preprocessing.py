...
class ImagePreprocessing:
    ...
    def check_resolution(self, image):
        ...
        if dpi_x < self.dpi_threshold or dpi_y < self.dpi_threshold:
            print("Low image resolution (DPI).")
            indicator=1
            new_image = Image.new("RGB", image.size)
            new_image.paste(image)
            new_image.info["dpi"] = (600, 600)
            return new_image, indicator
    ...
    def check_clarity(self, image):
        ...
        laplacian = cv2.Laplacian(np.array(gray), cv2.CV_64F)
        return 1 if laplacian.var() < self.clarity_threshold else 0

    def check_noise(self, image):
        noise = np.mean(np.array(image))
        return 1 if noise > self.noise_threshold else 0
